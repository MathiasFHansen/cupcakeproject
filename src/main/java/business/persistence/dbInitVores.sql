...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cupcake`.`cup_cake_top` (
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `cupcake`.`cup_cake_bund` (
  `name` VARCHAR(45) NULL,
...
CREATE TABLE IF NOT EXISTS `cupcake`.`ordre` (
  ...
  CONSTRAINT `fk_ordre_cup_cake_bund1`
    ...
    REFERENCES `cupcake`.`cup_cake_bund` (`cup_cake_bund_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordre_cup_cake_top1`
    FOREIGN KEY (`cup_cake_top_id`)
    REFERENCES `cupcake`.`cup_cake_top` (`cup_cake_top_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordre_indkøbskurv1`
    FOREIGN KEY (`indkøbskurv_id`)
    REFERENCES `cupcake`.`indkøbskurv` (`indkøbskurv_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
